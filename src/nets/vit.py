...
class VisionTransformer(nn.Module):
    """ Vision Transformer with support for patch or hybrid CNN input stage
        Vision Transformer 与支持补丁或混合CNN输入阶段
    """
...
def get_vit(embedding_size, pretrained):
    if pretrained:
        raise ValueError("No pretrained model for vit")
    return VisionTransformer(embedding_size)
                #  img_size: int = 112,
                #  patch_size: int = 16,
                #  in_channels: int = 3,
                #  num_classes: int = 1000,
...
# embedding_size, img_size, patch_size, in_channels, num_classes, embed_dim, depth, num_heads, mlp_ratio, qkv_bias, qk_scale, drop_rate,
